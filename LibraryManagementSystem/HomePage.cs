    ...
    public partial class HomePage : Form
    {
        ...
        private void HomePage_Load(object sender, EventArgs e)
        {
            ...
            if (File.Exists(filePath))
            {
                ...
                // Перевіряємо наявність таблиці 'Видача та повернення книг' і рахуємо кількість рядків
                if (ds.Tables.Contains("IssueReturnBooks"))
                {
                    countIssueBooks = ds.Tables["IssueReturnBooks"].Rows.Count;
                ...
                if (ds.Tables.Contains("IssueReturnBooks"))
                {
                    ...
                    foreach (DataRow row in issueReturnTable.Rows)
                    {
                        if (row["ReturnStatus"].ToString() == "Не повернено")
                        {
                            countBorrowsBooks++;
                        }
                    }
                    countBorrows.Text = countBorrowsBooks.ToString();
                }
                else
                {
        ...
        {
            if (ds.Tables.Contains("IssueReturnBooks") && ds.Tables.Contains("Readers"))
            {
                var readerVisits = new Dictionary<string, int>();
                var readerNames = new Dictionary<string, string>();

                // Збираємо дані про імена читачів
                foreach (DataRow row in ds.Tables["Readers"].Rows)
                {
                    string readerID = row["ReaderID"].ToString();
                    string readerName = row["ReaderName"].ToString();
                    readerNames[readerID] = readerName;
                }

                // Збираємо дані про відвідування читачів
                foreach (DataRow row in ds.Tables["IssueReturnBooks"].Rows)
                {
                    string readerID = row["ReaderID"].ToString();
                ...
                {
                    string readerID = item.Key;
                    string readerName = readerNames.ContainsKey(readerID) ? readerNames[readerID] : "Unknown";
                    gridViewPopularReaders.Rows.Add(readerName, item.Value);
                }
            }
            else
            {
